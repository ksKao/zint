...
import { widgetConfigSchema } from "@/lib/types/widget.type";
...
const transactionBackupSchema = z.array(
  z.object({
    id: z.cuid2(),
    name: z.string(),
    currency: z.string(),
    ...
    transactions: z.array(
      z.object({
        ...
        subCategoryId: z.cuid2().nullable(),
      }),
    ),
    widgets: z.array(
      z.object({
...
export default function BackupData() {
  ...
  const [confirmRestoreDialogOpen, setConfirmRestoreDialogOpen] =
    useState(false);
  const {
    data: file,
    isError,
  ...
  const {
    ...
    mutationFn: async () => {
      ...
      const backup = await db.query.accounts.findMany({
        with: {
          categories: {
            with: {
              subCategories: true,
      ...
      // call parse here to strip the extra fields
      const parsed = transactionBackupSchema.parse(backup);

      const mediaFile = new File([JSON.stringify(parsed)], "media.json", {
  ...
  const { mutate: restoreBackup, isPending: restoreBackupPending } =
    useMutation({
      mutationFn: async () => {
        ...
        const backup = transactionBackupSchema.parse(json);

        // deleting accounts will cause all tables to cascade delete
        ...
        for (const account of backup) {
          if (account.widgets.length)
  ...
  return (
    <>
      <AlertDialog
        ...
        onOpenChange={setConfirmRestoreDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <p>
            <b>Restoring backup will cause all data to be overwritten.</b> It is
            recommended to export your data first.
          </p>
              ...
              <Item variant="outline">
                ...
                <ItemActions>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setConfirmRestoreDialogOpen(true)}
                  >
                    Restore
                  </Button>
                </ItemActions>
